...
public class task5 {
	public static void main(String[] args){
		...
		System.out.println(digitsCount(654));
		System.out.println(totalPoints(new String[]{"cat", "create", "sat"}, "caster"));
		System.out.println(longestRun(new int[]{1,2,3,4,5,7,6,5,4,3,2,1}));
		...
		System.out.println(spiderVsFly("23","41"));
	}
	...
	{
		int rez=0;
		Map<String, Long> slovo=str.chars().mapToObj(s->String.valueOf((char) s))
		.collect(Collectors.groupingBy(x->x, Collectors.counting()));
		for (String vars : a) 
		{
			Map<String, Long> l=vars.chars().mapToObj(s->String.valueOf((char) s))
			.collect(Collectors.groupingBy(x->x, Collectors.counting()));
			long col=l.entrySet().stream().filter(x->slovo.containsKey(x.getKey()) && slovo.get(x.getKey())>=x.getValue()).count();
			if(col==vars.length())
			{
				rez=rez+((int) col-2);
	...
	public static String rearrange(String s)
	{
		...
		for(int i=0; i<s.split(" ").length; i++)
		{
			...
			{
				String a=j.next();
				if(i==a.chars().mapToObj(x->(char) x).filter(m->Character.isDigit(m))
					.mapToInt(l->l-'0').findFirst().getAsInt()-1)
				{
